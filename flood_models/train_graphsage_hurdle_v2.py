...
[1] LayerNorm replaces BatchNorm1d.
    train_partial_obs_fixed.py's own docstring already diagnosed this failure
    mode: BatchNorm erases columns that are near-constant WITHIN one event
    (rain_* features have within-event std ~0.05-0.10 vs 0.6-1.5 for terrain
    features — almost flat per storm), which is exactly the column meant to
    carry storm-to-storm severity signal. v1's GraphSAGEHurdle used
    BatchNorm1d on hidden activations from batches mixing two different
    city-events — the same failure mode, one layer downstream of the raw
    features. LayerNorm normalizes per-node across the feature dimension
    instead, so it can't do this.

[2] SEPARATE masks for the two heads, instead of one shared mask (v1) or no
    mask at all (v1's --no-wse-mask experiment).
    v1's shared valid_mask gated BOTH heads by "SAR flooded AND FwDET
    assigned a WSE," which silently dropped ~70% of true positives from the
    classification head — build_graphs.py's own design note #3 already
    warns against exactly this. Running v1 with --no-wse-mask fixed
    classification (PR-AUC 0.130->0.146) but made depth WORSE (R2 -0.036 ->
    -0.043), because it then let the depth head train on flooded-but-
    unassigned nodes whose depth value is a meaningless zero-fill, not a
    real reading. The correct fix: classification trains on the full
    labeled set (cls_valid); depth stays gated to trustworthy rows only
    (depth_valid = dry OR flooded-with-assigned-WSE).

[3] log1p depth target for training, inverted (expm1, clamped >=0) only at
    evaluation.
    Depth targets are heavily right-skewed (mostly 0, a few events up to
    21m). Training MSE directly on that lets rare large values dominate the
    gradient on the shared trunk, which also feeds the classification head.
    Standard practice for skewed positive regression targets; MAE/RMSE/R2
    are still reported in metres, not log-metres, so numbers stay
    comparable to v1's.

ALSO ADDED: a HistGradientBoostingClassifier/Regressor tabular baseline,
trained and evaluated on the EXACT SAME train/test node split and masks
(features flattened, no graph structure) — the same technique family
(RF/XGBoost/LightGBM) the literature comparison used, so "does the graph
structure actually help" has an honest answer computed on this data,
not asserted.

NOT CLAIMED: beating any specific published paper's numbers. Different
dataset, different region, different task definition — not a valid
head-to-head. Only this run's own numbers, honestly reported.
"""
...
def parse_args():
    ...
    p.add_argument("--checkpoint-dir", default=os.path.join(HERE, "checkpoints_graphsage_v2"))
...
# ============================================================
# [2] Separate masks: classification gets the full labeled set,
#     depth stays gated to trustworthy rows only.
# ============================================================

def build_masks(data):
    feat_valid = ~torch.isnan(data.x).any(dim=1)
    cls_label_valid = ~torch.isnan(data.y_flooded)
    depth_label_valid = ~torch.isnan(data.y_depth)

    cls_valid = feat_valid & cls_label_valid
    depth_trustworthy = (data.y_flooded == 0) | data.mask_depth
    depth_valid = feat_valid & depth_label_valid & depth_trustworthy
    return cls_valid, depth_valid


for data in train_graphs + test_graphs:
    data.cls_valid, data.depth_valid = build_masks(data)
    # kept for compatibility with any code expecting a single mask name
    data.valid_mask = data.cls_valid

# ============================================================
# Normalization — fit on train cls_valid rows (same convention as v1)
# ============================================================
train_x_valid = torch.cat([d.x[d.cls_valid] for d in train_graphs], dim=0)
...
def normalize_and_clean(data):
    ...
    # [3] log1p target for training; kept alongside the raw metres version
    data.y_depth_log = torch.log1p(data.y_depth.clamp(min=0))
    return data
...
for d, name in zip(train_graphs, train_names):
    print(f"  {name}: cls_valid {d.cls_valid.sum().item()}/{d.num_nodes}  "
          f"depth_valid {d.depth_valid.sum().item()}/{d.num_nodes}")
...
# ============================================================
# [1] Model — LayerNorm instead of BatchNorm1d
# ============================================================
...
class GraphSAGEHurdle(torch.nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        depth_log = self.depth_head(x).squeeze(-1)
        return cls_logit, depth_log
...
flooded_count = sum((d.y_flooded[d.cls_valid] == 1).sum().item() for d in fit_split)
dry_count = sum((d.y_flooded[d.cls_valid] == 0).sum().item() for d in fit_split)
...
axes[0].set_xlabel("epoch"); axes[0].set_ylabel("loss (focal + MSE on log1p depth)"); axes[0].legend()
...
axes[1].set_title(f"Recall @ 0.5 threshold (model selection metric)")
plt.tight_layout()
...
def evaluate(loader, names, cls_threshold=0.5):
    ...
    with torch.no_grad():
        for data, name in zip(loader, names):
            data = data.to(device)
            cls_logit, depth_log_pred = model(data.x, data.edge_index)
            cmask, dmask = data.cls_valid, data.depth_valid

            prob = torch.sigmoid(cls_logit[cmask]).cpu().numpy()
            t_flooded = data.y_flooded[cmask].cpu().numpy()

            d_pred = torch.expm1(depth_log_pred[dmask]).clamp(min=0).cpu().numpy()
            t_depth = data.y_depth[dmask].cpu().numpy()

            mae = np.mean(np.abs(d_pred - t_depth)) if len(t_depth) else float("nan")
            rmse = np.sqrt(np.mean((d_pred - t_depth) ** 2)) if len(t_depth) else float("nan")
            ss_res = np.sum((t_depth - d_pred) ** 2)
            ss_tot = np.sum((t_depth - t_depth.mean()) ** 2) if len(t_depth) else 0.0
            ...
            roc_auc = (roc_auc_score(t_flooded, prob)
                      if t_flooded.sum() > 0 and t_flooded.sum() < len(t_flooded) else float("nan"))

            results.append({
                "event": name, "n_cls_nodes": cmask.sum().item(), "n_depth_nodes": dmask.sum().item(),
                "MAE_m": mae, "RMSE_m": rmse, "R2": r2,
                "PR_AUC": pr_auc, "ROC_AUC": roc_auc, "recall": recall, "precision": precision, "F1": f1,
    ...
    overall_roc_auc = (roc_auc_score(all_true_flooded, all_prob)
                       if all_true_flooded.sum() > 0 else float("nan"))
    ...
    print(f"OVERALL flood detection: PR-AUC={overall_pr_auc:.3f} | ROC-AUC={overall_roc_auc:.3f} | "
          f"recall={overall_recall:.3f} | precision={overall_precision:.3f} | F1={overall_f1:.3f}  "
          f"(@ threshold {cls_threshold})")
...
precisions_c, recalls_c, thresholds_c = precision_recall_curve(test_true_flooded, test_prob)
...
plt.title("Precision-recall tradeoff across thresholds (GraphSAGE v2)")
...
plt.xlabel("actual flood_depth_m"); plt.ylabel("predicted flood_depth_m (depth head, expm1-inverted)")
plt.title("GraphSAGE v2 -- predicted vs actual depth (held-out cyclones)")
scatter_path = os.path.join(args.checkpoint_dir, "depth_scatter.png")
plt.savefig(scatter_path, dpi=120)
print("saved ->", scatter_path)

# ============================================================
# Tabular baseline — HistGradientBoosting, same split, same masks,
# no graph structure. Answers "does the graph actually help."
# ============================================================
print("\n" + "=" * 72)
print("TABULAR BASELINE (HistGradientBoosting, no graph structure)")
print("Same train/test event split, same cls_valid/depth_valid masks.")
print("=" * 72)

Xtr_cls = np.concatenate([d.x[d.cls_valid].numpy() for d in train_graphs])
ytr_cls = np.concatenate([d.y_flooded[d.cls_valid].numpy() for d in train_graphs]).astype(int)

base = ytr_cls.mean()
pos_w = (1 - base) / max(base, 1e-9)
sw_cls = np.where(ytr_cls == 1, pos_w, 1.0)

gbm_cls = HistGradientBoostingClassifier(max_iter=200, max_depth=6, learning_rate=0.08,
                                         random_state=42)
gbm_cls.fit(Xtr_cls, ytr_cls, sample_weight=sw_cls)

Xtr_depth = np.concatenate([d.x[d.depth_valid].numpy() for d in train_graphs])
ytr_depth_log = np.concatenate([d.y_depth_log[d.depth_valid].numpy() for d in train_graphs])
gbm_depth = HistGradientBoostingRegressor(max_iter=200, max_depth=6, learning_rate=0.08,
                                          random_state=42)
gbm_depth.fit(Xtr_depth, ytr_depth_log)

base_results = []
base_prob_all, base_true_flooded_all = [], []
base_depth_pred_all, base_true_depth_all = [], []
for d, name in zip(test_graphs, test_names):
    Xte_cls = d.x[d.cls_valid].numpy()
    yte_cls = d.y_flooded[d.cls_valid].numpy()
    p = gbm_cls.predict_proba(Xte_cls)[:, 1]

    Xte_depth = d.x[d.depth_valid].numpy()
    yte_depth = d.y_depth[d.depth_valid].numpy()
    dp = np.expm1(gbm_depth.predict(Xte_depth)).clip(min=0)

    pr_auc = average_precision_score(yte_cls, p) if yte_cls.sum() > 0 else float("nan")
    roc_auc = (roc_auc_score(yte_cls, p)
              if yte_cls.sum() > 0 and yte_cls.sum() < len(yte_cls) else float("nan"))
    pred = (p >= 0.5).astype(int)
    tp = int(((pred == 1) & (yte_cls == 1)).sum())
    fp = int(((pred == 1) & (yte_cls == 0)).sum())
    fn = int(((pred == 0) & (yte_cls == 1)).sum())
    recall = tp / (tp + fn) if (tp + fn) > 0 else float("nan")
    precision = tp / (tp + fp) if (tp + fp) > 0 else float("nan")
    f1 = (2 * precision * recall / (precision + recall)
          if (precision + recall) > 0 and not np.isnan(precision) and not np.isnan(recall)
    ...
    mae = np.mean(np.abs(dp - yte_depth)) if len(yte_depth) else float("nan")
    ss_res = np.sum((yte_depth - dp) ** 2)
    ss_tot = np.sum((yte_depth - yte_depth.mean()) ** 2) if len(yte_depth) else 0.0
    r2 = 1 - ss_res / ss_tot if ss_tot > 0 else float("nan")

    base_results.append({"event": name, "PR_AUC": pr_auc, "ROC_AUC": roc_auc, "recall": recall,
                         "precision": precision, "F1": f1, "MAE_m": mae, "R2": r2})
    base_prob_all.append(p); base_true_flooded_all.append(yte_cls)
    base_depth_pred_all.append(dp); base_true_depth_all.append(yte_depth)

base_df = pd.DataFrame(base_results)
print(base_df.to_string(index=False))

base_prob_all = np.concatenate(base_prob_all)
base_true_flooded_all = np.concatenate(base_true_flooded_all)
base_depth_pred_all = np.concatenate(base_depth_pred_all)
base_true_depth_all = np.concatenate(base_true_depth_all)

base_overall_pr_auc = average_precision_score(base_true_flooded_all, base_prob_all)
base_overall_roc_auc = roc_auc_score(base_true_flooded_all, base_prob_all)
base_overall_mae = np.mean(np.abs(base_depth_pred_all - base_true_depth_all))
ss_res = np.sum((base_true_depth_all - base_depth_pred_all) ** 2)
ss_tot = np.sum((base_true_depth_all - base_true_depth_all.mean()) ** 2)
base_overall_r2 = 1 - ss_res / ss_tot if ss_tot > 0 else float("nan")

print(f"\nBASELINE OVERALL: PR-AUC={base_overall_pr_auc:.4f}  ROC-AUC={base_overall_roc_auc:.4f}  "
      f"MAE={base_overall_mae:.4f} m  R2={base_overall_r2:.4f}")

print("\n" + "=" * 72)
print("HEAD-TO-HEAD (this run's own numbers, same data, same split)")
print("=" * 72)
print(f"{'':20s}{'PR-AUC':>10s}{'ROC-AUC':>10s}{'depth MAE_m':>14s}{'depth R2':>12s}")
overall_pr_auc = average_precision_score(test_true_flooded, test_prob)
overall_roc_auc = roc_auc_score(test_true_flooded, test_prob)
overall_mae = np.mean(np.abs(test_depth_pred - test_true_depth))
ss_res = np.sum((test_true_depth - test_depth_pred) ** 2)
ss_tot = np.sum((test_true_depth - test_true_depth.mean()) ** 2)
overall_r2 = 1 - ss_res / ss_tot if ss_tot > 0 else float("nan")
print(f"{'GraphSAGE v2':20s}{overall_pr_auc:10.4f}{overall_roc_auc:10.4f}{overall_mae:14.4f}{overall_r2:12.4f}")
print(f"{'HistGBM (tabular)':20s}{base_overall_pr_auc:10.4f}{base_overall_roc_auc:10.4f}{base_overall_mae:14.4f}{base_overall_r2:12.4f}")
print(f"{'lift over base rate':20s}{overall_pr_auc/max(test_true_flooded.mean(),1e-9):9.2f}x")

# ============================================================
# Save checkpoint + metrics
# ============================================================
timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
ckpt_path = os.path.join(args.checkpoint_dir, f"graphsage_hurdle_v2_{timestamp}.pt")
...
metrics_path = os.path.join(args.checkpoint_dir, f"graphsage_hurdle_v2_metrics_{timestamp}.csv")
test_metrics_df.to_csv(metrics_path, index=False)
base_metrics_path = os.path.join(args.checkpoint_dir, f"baseline_gbm_metrics_{timestamp}.csv")
base_df.to_csv(base_metrics_path, index=False)
print("saved metrics ->", metrics_path)
print("saved baseline metrics ->", base_metrics_path)
